...
class VirusTotal(Module, common.PackageManager, common.ClassLoader, common.FileSystem):
    ...
    def execute(self, arguments):
        ...
        for scan in jsonfile['scans']:
            self.stdout.write(
                '\n' + scan + ' - ' + 'Detected=' + str(jsonfile['scans'][scan]['detected']) + '\tResult=' + str(
                    jsonfile['scans'][scan]['result']) + '\tLast Scanned=' + str(jsonfile['scans'][scan]['update']))
    ...
    def checkMD5(self, md5):
        ...
        result = request.urlopen(url, data.encode())
